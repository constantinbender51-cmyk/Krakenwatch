    ...
    <script>
        ...
        function renderTopChart(dataPoints, color, isComposite) {
            ...
            if (isComposite && dataPoints.length > 0) {
                // Add Reference Lines (+5% / -5%) as Datasets
                const startX = dataPoints[0].x;
                const endX = dataPoints[dataPoints.length-1].x;
                
                const lineStyle = {
                    borderColor: '#000000', borderWidth: 1, pointRadius: 0, borderDash: [] // Solid black lines
                };
            ...
            chartInstances.top = new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: { legend: { display: false }, tooltip: { enabled: false } }, // No tooltips for cleaner look
                    scales: {
                        x: { display: false }, // Completely hide X axis
                        y: { display: false }  // Completely hide Y axis (lines handled by datasets)
                    },
                    layout: { padding: 0 }
                }
            });
            ...
            chartInstances.bottom = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            display: true, // Keep "visible" for the grid
                            drawBorder: false, // Hide the spine
                            ticks: { display: false }, // Hide numbers
                            grid: {
                                // Only draw the grid line if value is 0 (Midline)
